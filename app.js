...
let database = null;
const initializeResponseAndServer = async () => {
  try {
    database = await open({
...
app.get("/players/:playerId/", async (request, response) => {
  ...
  const getPlayersIdQuery = `
        ...
        WHERE 
            player_id  =${playerId}  ;`;
  const player = await database.get(getPlayersIdQuery);
...
app.get("/players/:playerId/playerScores/", async (request, response) => {
  ...
  const getPlayerDetails = `
        ...
        WHERE 
            player_id = ${playerId} ;`;
  const details = await database.get(getPlayerDetails);
